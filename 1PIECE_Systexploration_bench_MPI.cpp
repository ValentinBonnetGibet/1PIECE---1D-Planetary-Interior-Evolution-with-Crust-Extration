...
double Tm0_init,Tm0_end,eta0_init,eta0_end;
...
lecture_explo(explor_param,Tm0_init,Tm0_end,eta0_init,eta0_end,N_k0,N_eta0);
...
Eigen::VectorXd Tm0_vec = Eigen::VectorXd::Ones(N_k0);
...
std::fprintf(resume, "Nbr run   Tm0  eta0    Tm  Tc  Tcr_max_N   Tcr_max_S   Dl_N    Dl_S    Dc_N    Dc_S    LMBD_ath    LMBD_lith_N LMBD_lith_S LMBD_cr_N   LMBD_cr_S \n");
fclose(resume);
}
else {
    std::string line,tmp ;
...
std::cout << rank << " : rank," << i <<  " / " << N_tot << " done" << "  eta0 = " << eta0_exp(eta_i) << "  Tm0 = " << Tm0_vec(k_i) << "         nom dossier :  " << dossier_time << std::endl;
